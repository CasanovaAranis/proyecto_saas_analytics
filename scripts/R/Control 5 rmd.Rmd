---
title: "Control 5 - Marketing Analytics"
...
install.packages(c("tidyverse", "plotly", "scales", "lubridate",
                  "corrplot", "gridExtra", "hrbrthemes", "stats", "flexdashboard"))
```
```{r}
# Cargar librerías
...
analizar_correlaciones <- function(datos) {
  ...
  columnas_excluir <- c("year", "month", "quarter")
  metricas_numericas <- datos %>%
    select_if(is.numeric) %>%
    select(-any_of(columnas_excluir))
  
  # Validar que existan suficientes columnas numéricas
  if (ncol(metricas_numericas) < 2) {
